...
def objective(x, p, v2, v3, v4):
    ...
    E3 = np.exp(-1j * E3)
    E4 = np.einsum("m,ijklm->ijkl", Gamma, v4)
    E4 = np.exp(-1j * E4)

    # 1-local
    if p ==1:
        H1 = np.einsum("b,ab->a", f_pre, E2) ** 2
        res = np.einsum("a,b,a,b,a,b,ab->", a[:, p], a[:, p], f_pre, f_pre, H1, H1, E2)
        return 0.5 * res.real

    # 2-local
    if p == 2:
        # edge 1
        U1 = np.einsum("b,ab->a", f_pre, E2) ** 2
        U2 = np.einsum("b,b,ab->a", f_pre, U1, E2)
        L1 = np.einsum("b,ab->a", f_pre, E2)

        r1 = np.einsum("a,b,c,a,c,abc->ac", f_pre, f_pre, f_pre, L1, L1, E3)
        res1 = np.einsum("A,B, A,B, A,B, ac, ABac->", a[:, p], a[:, p], f_pre, f_pre, U2, U2, r1, E4)

        # edge 2
        H2 = np.einsum("b,b,ab->a", f_pre, U1, E2)**2
        res2 = np.einsum("a,b,a,b,a,b,ab->", a[:, p], a[:, p], f_pre, f_pre, H2, H2, E2)

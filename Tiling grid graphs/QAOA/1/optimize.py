...
if __name__ == '__main__':
    ...
    v2 = a[:, None] * a
    v3 = a[:, None, None] * a[:, None] + a[:, None] * a # ab+bc
    v4 = a[:, None, None, None] * a[:, None, None] + a[:, None, None, None] * a[:, None] + a[:, None, None] * a #ABac (AB+Aa+Bc)
    ...
    with open(out_file, "a") as f:
        ...
        for gamma0, beta0 in zip(gamma, beta):
            ...
            f.write("cut fraction: %s\n" %(str(0.5-objective(x0,p,v2,v3,v4)),))
            f.flush()

            # optimize the objective function
            finish = 0
            res = minimize(objective, x0, args=(p,v2,v3,v4), bounds=bounds,callback=callback)

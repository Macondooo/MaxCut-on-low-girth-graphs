...
def objective(x, p, pre_E, pre_E_ring):
    ...
    # pre-calculation
    E1 = np.einsum("k,ijk->ij", Gamma1, pre_E)
    E1 = np.exp(-1j * E1)
    E2 = np.einsum("k,ijk->ij", Gamma2, pre_E)
    E2 = np.exp(-1j * E2)
    ...
    # calculation of G and H
    G[1] = np.einsum("b,ab->a", f_pre, E2)**2
    tmp1 = np.einsum("b,ab->a",f_pre, E1)**2
    H[1] = np.einsum("b,ab,a->a",f_pre,E2,tmp1)
    if p == 2 or p == 3:
        G[2] = np.einsum("b,b,ab->a",H[1],f_pre,E2)**2
        tmp2 = np.einsum("b,b,ab->a",H[1],f_pre,E2)
        H[2] = np.einsum("b,c,b,c,a,abc->a", f_pre, f_pre, G[1], G[1], tmp2, E_ring)
        if p == 3: 
            G[3] = np.einsum("b,b,ab->a", f_pre, H[2], E2) ** 2
            tmp3  = np.einsum("b,b,ab->a", f_pre, H[2], E2)
    ...
    # second kind of edge
    res2 = np.einsum(
        "i,j,i,j,i,j,ij->", a[:, p], a[:, p], f_pre, f_pre, H[p], H[p], E2)
